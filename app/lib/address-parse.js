  ...
  if (item.children) {
    item.children.forEach((city, cityIndex) => {
      const index = filterCity.findIndex(
        filter => city.name.indexOf(filter) !== -1
      );
      if (index !== -1) {
...
const cities = addressJson.reduce((per, cur) => {
  return per.concat(
    cur.children
      ? cur.children.map(({ children, ...others }) => ({
          ...others,
  ...
  return per.concat(
    cur.children
      ? cur.children.reduce((p, c) => {
          ...
          return p.concat(
            c.children
              ? c.children.map(({ children, ...others }) => ({
                  ...others,
                  cityCode,
                  provinceCode,
                }))
...
const AddressParse = (address, options) => {
  ...
  if (detail && detail.length > 0) {
    ...
    const index = copyDetail.findIndex(item =>
      judgeFragmentIsName(item, nameMaxLength)
    );
    let name = '';
    if (index !== -1) {
    ...
    if (name) {
      ...
      detail.splice(
        detail.findIndex(item => item === name),
        1
      );
    }
  }
  ...
  if (city.length === 0) {
    for (let i = 1; i < fragment.length; i++) {
      ...
      const regexCity = new RegExp(
        `\{\"code\":\"[0-9]{1,6}\",\"name\":\"${str}[\u4E00-\u9FA5]*?\",\"provinceCode\":\"${
          province[0] ? `${province[0].code}` : '[0-9]{1,6}'
        }\"\}`,
        'g'
      );
  ...
  if (area.length === 0) {
    for (let i = 1; i < fragment.length; i++) {
      ...
      const regexArea = new RegExp(
        `\{\"code\":\"[0-9]{1,9}\",\"name\":\"${str}[\u4E00-\u9FA5]*?\",\"cityCode\":\"${
          city[0] ? city[0].code : '[0-9]{1,6}'
        }\",\"provinceCode\":\"${
          province[0] ? `${province[0].code}` : '[0-9]{1,6}'
        }\"\}`,
        'g'
