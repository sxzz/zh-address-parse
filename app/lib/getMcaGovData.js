    ...
    try {
      http.get(this.sourceUrl, res => {
        ...
        res.on('end', () => {
          ...
          elementsArea = elementsArea.filter((index, item) =>
            $(item).text().trim()
          );

          let elementsProAndCity = $('.' + this.headerClass);
          elementsProAndCity = elementsProAndCity.filter((index, item) =>
            $(item).text().trim()
          );
          ...
          for (let i = 0; i <= elementsProAndCity.length; i += 2) {
            ...
            const next = $(elementsProAndCity[i + 1])
              .text()
              .trim();
            ...
            do {
              ...
              if (/\d/.test(codeOrName)) {
                if (regExp.test(codeOrName)) {
                  ...
                  const prefixCityCode = codeOrName.substring(2, 4);
                  regExp = new RegExp(
                    `^${prefixProvinceCode}${prefixCityCode}`
                  );
          ...
          if (parseTotal === total) {
            fs.writeFile(
              path.join(__dirname, 'provinceList.json'),
              JSON.stringify(listProvince),
              function (err) {
                if (err) return;
                console.log('导出成功');
              }
